...
void Twiddle(PID &pid, PID &otherPid, uWS::WebSocket<uWS::SERVER>& ws)
{
  ...
  if(is_running)
  {
    cout << "Twiddle Calculating. Value: " << pid.value << " Error:" << pid.average_error << " " << pid.count << "/" << TWIDDLE_MAX_FRAME_COUNT << " dt: " << pid.dt;
    cout << " perror: " << pid.p_error << " ierror: " << pid.i_error << " derror: " << pid.d_error << endl;
